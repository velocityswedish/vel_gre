"""
Facebook Reels Automation - Bilingual English/Greek Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Greek branding
"""
...
# Greek translations for display
CATEGORIES_GREEK = {
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    category_greek = CATEGORIES_GREEK[category_english]
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 3} unique {category_english} phrases for English speakers learning Greek.
...
2. Greek translation (with commas matching the rhythm)
...
Write Greek text in actual Greek letters (e.g., "Ελπίδα" not "Elpida")."""

            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are a Greek teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "greek": "Πίστεψε στον εαυτό σου.", "pronunciation": "PEE-stehv-seh ston eh-af-TOH soo."},
            {"english": "You are capable of amazing things.", "greek": "Είσαι ικανός για καταπληκτικά πράγματα.", "pronunciation": "EE-seh i-ka-NOS ya ka-ta-plik-TIKA PRAG-ma-ta."},
            {"english": "Dream big, start small.", "greek": "Ονειρεύσου μεγάλα, ξεκίνα με μικρά βήματα.", "pronunciation": "O-nee-REV-soo me-GA-la, khee-KEE-na meh mee-KRA VEE-ma-ta."},
            {"english": "Your future is created by your actions.", "greek": "Το μέλλον σου δημιουργείται από τις πράξεις σου.", "pronunciation": "To MEH-lon soo dee-mee-oor-YEE-teh ah-po tees PRAK-sees soo."},
            {"english": "Never give up on your dreams.", "greek": "Μην εγκαταλείπεις ποτέ τα όνειρά σου.", "pronunciation": "Meen en-ga-ta-LEE-pees PO-teh ta O-nee-RA soo."},
        ],
        "Love": [
            {"english": "Love yourself first.", "greek": "Αγάπησε πρώτα τον εαυτό σου.", "pronunciation": "A-GA-pee-seh PRO-ta ton eh-af-TOH soo."},
            {"english": "Love makes everything possible.", "greek": "Η αγάπη κάνει τα πάντα δυνατά.", "pronunciation": "Ee a-GA-pee KA-nee ta PAN-da dee-na-TA."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        # Generate Greek audio
        el_success = asyncio.run(generate_single_audio(phrase["greek"], GREEK_VOICE, str(greek_file)))
        if el_success:
            print(f"    ✓ Greek: {greek_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(greek_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        el_duration = get_audio_duration(str(greek_file))

        # Add pause between English and Greek
        pause_between = 0.5
        total_duration = en_duration + pause_between + el_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + EL: {el_duration:.2f}s)")
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{greek_file.as_posix()}'\n")
        ...
        audio_files.append({
            ...
            "greek": str(greek_file),
            "combined": str(combined_file),
            "duration": actual_duration,
            "en_duration": en_duration,
            "el_duration": el_duration
        })
...
def create_impressive_background(category_english: str):
    ...
    # HIGH CONTRAST gradients for ALL 25 categories (Greek names)
    category_colors = {
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    # Windows fonts
    if sys.platform == "win32":
        font_category = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 60)
        font_large = ImageFont.truetype("C:/Windows/Fonts/arialbd.ttf", 85)
        font_pronunciation = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 42)
        font_branding = ImageFont.truetype("C:/Windows/Fonts/arialbd.ttf", 52)
    else:
        # Linux fonts
        font_category = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 60)
        font_large = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 85)
        font_pronunciation = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 42)
        font_branding = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 52)

    english = phrase_data.get("english", "")
    greek = phrase_data.get("greek", "")
    ...
    # Category at top
    category_text = CATEGORIES_GREEK[category_english].upper()
    ...
    # Greek text
    greek_y = english_y + total_height + 110  # Increased from 100
    greek_lines = wrap_text(greek, font_large, VIDEO_WIDTH - 140)
    total_height = len(greek_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, greek_y - 55), (VIDEO_WIDTH - 60, greek_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(greek_lines):
        y_pos = greek_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = greek_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_GREEK[category_english]})")
    print(f"{'='*80}\n")
    ...
    for i, phrase in enumerate(phrases, 1):
        print(f"  {i}. {phrase['english']} → {phrase['greek']}")
    ...
    print("\n[3/4] Generating audio (English + Greek with 500ms pause)...")
    ...
    print(f"  🏷️  Branding: Velocity Greek")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇬🇷 VELOCITY GREEK - FACEBOOK REELS AUTOMATION 🇬🇷")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Greek branding at bottom")
    ...
    for i, cat in enumerate(CATEGORIES_ENGLISH, 1):
        print(f"   {i:2d}. {cat} ({CATEGORIES_GREEK[cat]})")
    print(f"\n📅 DAILY CAPACITY:")
